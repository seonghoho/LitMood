...
- [x] 컬렉션 CRUD, 아이템 추가/제거/정렬, 큐레이터 노트 (F-05)
- [x] 공개 컬렉션 페이지 (SSR + JSON-LD) + `next/og` 동적 OG 이미지
- [x] 무드별 탐색 `/moods/{name}` (F-07-01)
- [x] 프로필에 컬렉션 목록, 기록의 무드 칩 → 탐색 링크

**완료 조건**: 컬렉션 링크를 카카오톡에 붙였을 때 커버·제목·아이템 수가 담긴 카드가 뜬다
→ 달성. 1200×630 OG 이미지가 제목·설명·표지·작성자·개수를 담아 생성됨을 확인했다.

---

## M4. 소셜

- [x] 팔로우 / 팔로잉 피드 / 좋아요 (F-06-01~03)
- [x] 신고·차단 (F-06-05)
- [x] 인기 콘텐츠 랭킹 — Redis Sorted Set (F-07-02)
- [x] **FOLLOWERS 공개범위가 비로소 실제로 동작** — M2·M3 에서 `false` 로 고정돼 있던 부분

**완료 조건**: 팔로우한 사용자의 FOLLOWERS 기록이 피드와 프로필에 나타난다. → 달성.
