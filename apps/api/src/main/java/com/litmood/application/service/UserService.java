...
public class UserService {
    ...
    public User updateProfile(
        ...
        if (avatarUrl != null) {
            ...
            if (avatarUrl.isEmpty()) {
                // 빈 문자열은 "지운다" 다 — 리뷰·소개와 같은 규칙이다 (이슈 #5)
            ...
            if (previous != null && !previous.equals(user.getAvatarUrl())) {
                discard(previous);
            }
        }
    ...
    /**
     * 더 이상 참조되지 않는 아바타 객체를 지운다.
     *
     * <p>실패해도 삼킨다. 정리는 부가 작업이고, 스토리지가 응답하지 않는다고 프로필 저장까지
     * 실패시킬 이유가 없다 — 자격증명이 없어도 API 가 기동하게 해 둔 것과 같은 판단이다.
     * 대신 새는 것을 모르고 지나가지 않도록 로그를 남긴다.
     */
    private void discard(String publicUrl) {
        try {
            avatarStorage.deleteAvatar(publicUrl);
        } catch (Exception e) {
            log.warn("이전 아바타를 지우지 못했습니다 — 객체가 남습니다: {} ({})", publicUrl, e.getMessage());
        }
    }
