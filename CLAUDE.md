...
| 무드로 걸렀는데 0건이 나옴            | 서버는 정규화된 `name`("비오는날")으로 대조합니다. 화면에 보이는 `displayName`("비 오는 날")을 보내면 400 도 아니고 조용히 0건입니다         |
| 이미지가 상자를 안 채움               | 리셋의 `img { height: auto }` 가 유틸리티 클래스를 이깁니다. 대체 요소는 `top/bottom: 0` 으로도 늘어나지 않으니 크기를 인라인으로 못박으세요 |
